...
var successMsg = [ "Esaye encore échoue de nouveau échoue mieux","Félicitations","Tu es un super hero","Je ne perde jamais soit je gagne soit j'apprends","Puisse le bonheur submerger ta demeure","success","Puisse le bonheur et la réussite t’accompagner","harmonie","honneur","bon travail"];

var rand = Math.floor(Math.random() * (successMsg.length));
	...
	for (i = 0; i < MainHex.blocks.length; i++) {
		...
		for (j = 0; j < MainHex.blocks[i].length; j++) {
			...
			if (block.deleted == 2) {
				...
				document.getElementById("msg").innerHTML = successMsg[Math.floor(Math.random() * (successMsg.length))];

				setTimeout(() => {
					var audioT = document.getElementById("audioDebuit");
				audioT.play();},500);
				setTimeout(() => {
